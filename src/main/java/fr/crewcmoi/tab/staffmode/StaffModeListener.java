 ...
 * réapplique automatiquement le mode incognito (tab affiché "Player") à sa
 * reconnexion, avant que {@link fr.crewcmoi.tab.listeners.TabListListener}
 * (priorité MONITOR) n'applique le rôle dans le tab — d'où la priorité NORMAL
 * ici, pour s'exécuter avant.
 * <p>
...
public class StaffModeListener implements Listener {
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        if (staffModeManager.isActive(player.getUniqueId())) {
            roleManager.setIncognito(player.getUniqueId(), true);
        }
    }
}
