...
public class BountyGuiManager {
    ...
    public void open(Player viewer, int page) {
        ...
        Inventory gui = Bukkit.createInventory(holder, BountyHolder.SIZE,
                ChatColor.translateAlternateColorCodes('&', "&4&lPrimes des joueurs"));
    ...
    public void render(Player viewer, BountyHolder holder) {
        ...
        for (int i = start; i < end; i++) {
            ...
            if (rawMeta instanceof SkullMeta skullMeta) {
                ...
                lore.add(ChatColor.translateAlternateColorCodes('&', "&7Prime totale : &a" + format.format(target.getTotalAmount()) + currency));
                lore.add(ChatColor.translateAlternateColorCodes('&', "&7Contributeur(s) : &e" + target.getContributorCount()));
                lore.add("");
                ...
                for (BountyEntry entry : entries) {
                    if (shown >= 5) {
                        lore.add(ChatColor.translateAlternateColorCodes('&', "&8... et " + (entries.size() - shown) + " de plus"));
                        break;
                    }
                ...
                lore.add("");
                lore.add(ChatColor.translateAlternateColorCodes('&', "&cTue ce joueur pour récupérer la prime !"));
        ...
        if (targets.isEmpty()) {
            ...
            if (meta != null) {
                meta.setDisplayName(ChatColor.translateAlternateColorCodes('&', "&7Aucune prime active pour le moment."));
        ...
        if (page > 0) {
            gui.setItem(BountyHolder.PREV_PAGE_SLOT, createNavItem(Material.ARROW, "&ePage précédente"));
        }
        if (end < targets.size()) {
            gui.setItem(BountyHolder.NEXT_PAGE_SLOT, createNavItem(Material.ARROW, "&ePage suivante"));
        }
    ...
    private ItemStack createInfoItem(int currentPage, int totalPages, int totalTargets) {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.setDisplayName(ChatColor.translateAlternateColorCodes('&', "&4&lPrimes des joueurs"));
            List<String> lore = new ArrayList<>();
            lore.add(ChatColor.translateAlternateColorCodes('&', "&7Page &e" + currentPage + "&7/&e" + totalPages));
            lore.add(ChatColor.translateAlternateColorCodes('&', "&7Joueurs recherchés : &e" + totalTargets));
            lore.add(ChatColor.translateAlternateColorCodes('&', "&7Utilisez &f/bounty add <joueur> <montant>"));
            lore.add(ChatColor.translateAlternateColorCodes('&', "&7pour placer une prime."));
            meta.setLore(lore);
            item.setItemMeta(meta);
        }
        return item;
